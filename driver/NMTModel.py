...
class NeuralMT(nn.Module):
    def __init__(self, config, src_vocab_size, tgt_vocab_size, padding_idx):
        ...
        self.encoder = EncoderRNN(config.embed_size,
                                  config.hidden_size,
                                  config.num_layers,
    ...
    def encode(self, input, lengths=None):
    ...
    def forward(self, src_words, tgt_words, src_lengths):
        encoder_outputs, encoder_hidden = self.encode(src_words, src_lengths)
    ...
    def analyze(self, src_words, tgt_words, src_lengths):
        encoder_outputs, encoder_hidden = self.encode(src_words, src_lengths)
        decoder_init_hidden = self.decoder.init_decoder_state(encoder_hidden)
    ...
    def compute_loss(self, tgt_inputs, logits):
        ...
        stats = self.stats(loss.data[0], scores.data, target.data)
        return loss, stats
