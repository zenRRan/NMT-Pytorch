...
class NMTHelper(object):
    ...
    def src_data_id(self, src_input):
        ...
        return result + [self.src_vocab.END_SEQ]
    ...
    def train_one_batch(self, batch):
        ...
        decoder_outputs = self.model.forward(src_words, tgt_words, src_lengths)
    ...
    def analyze(self):
        ...
        for batch in create_batch_iter(self.valid_data, 1):
            src_words, tgt_words, src_lengths, tgt_lengths = self.pair_data_variable(batch)
            decoder_outputs, attn_scores = self.model.analyze(src_words, tgt_words, src_lengths)
            loss, stat = self.model.compute_loss(tgt_words, decoder_outputs)
    ...
    def translate(self, eval_data):
        ...
        for batch in create_batch_iter(eval_data, self.config.test_batch_size):
            ...
            allHyp, allScores = self.translate_batch(src_words, src_lengths)
    ...
    def translate_batch(self, src_inputs, src_input_lengths):
        ...
        encoder_outputs, encoder_hidden = self.model.encode(src_inputs, src_input_lengths)
